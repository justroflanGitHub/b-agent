...
async def execute_browser_task(task_id: str, goal: str, start_url: Optional[str], max_steps: int):
    ...
    try:
        ...
        # Execute task
        result = await agent.execute_task(goal=goal, start_url=start_url, max_steps=max_steps)

        # Update task progress
        if result.get("success"):
            task_manager.complete_task(
                task_id, success=True, extracted_data=result.get("extracted_data"), final_url=result.get("final_url")
            )
        else:
            task_manager.set_task_error(task_id, result.get("error", "Unknown error"), "execution_error")
...
async def lifespan(app: FastAPI):
    ...
    task_manager = TaskManager(max_concurrent_tasks=3, task_timeout=300.0, cleanup_interval=60.0)

    # Set task executor
    async def task_executor(task):
        await execute_browser_task(task.task_id, task.goal, task.start_url, task.max_steps)
...
def create_app(title: str = "Browser Agent API", version: str = "1.0.0", cors_origins: list = None) -> FastAPI:
    ...
    async def http_exception_handler(request, exc):
        return JSONResponse(
            ...
            content=ErrorResponse(error=exc.__class__.__name__, message=str(exc.detail)).model_dump(mode="json"),
        )
    ...
    # Task endpoints
    @app.post("/tasks", response_model=TaskStatus, status_code=201, tags=["Tasks"])
    async def create_task(request: TaskRequest, background_tasks: BackgroundTasks):
        ...
        task_manager.tasks.get(task_id)
        background_tasks.add_task(execute_browser_task, task_id, request.goal, request.start_url, request.max_steps)
    ...
    async def get_task_result(task_id: str):
        ...
        if status.status not in (TaskStatusEnum.COMPLETED, TaskStatusEnum.FAILED):
            raise HTTPException(status_code=400, detail=f"Task is not complete (status: {status.status})")
    ...
    async def cancel_task(task_id: str):
        ...
        if not task_manager.cancel_task(task_id):
            raise HTTPException(status_code=400, detail=f"Cannot cancel task with status {status.status}")

        return CancelResponse(
            task_id=task_id, status=TaskStatusEnum.CANCELLED, message=f"Task {task_id} cancelled successfully"
        )
    ...
    async def delete_task(task_id: str):
        ...
        if status.status == TaskStatusEnum.RUNNING:
            raise HTTPException(status_code=400, detail="Cannot delete running task. Cancel first.")
    ...
    async def get_metrics():
        ...
        return MetricsResponse(
            task_duration=[MetricPoint(timestamp=now, value=d) for d in metrics.get("task_durations", [])[-10:]],
            success_rate=[
                MetricPoint(
                    timestamp=now,
                    value=(
    ...
    async def execute_skill(skill_name: str, request: TaskRequest, background_tasks: BackgroundTasks):
        """
        ...
        if skill_name not in valid_skills:
            raise HTTPException(status_code=400, detail=f"Unknown skill: {skill_name}. Valid skills: {valid_skills}")
    ...
    async def store_credential(request: dict):
        ...
        """
        try:
            from browser_agent.security import CredentialType, RotationPolicy
