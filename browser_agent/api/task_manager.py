...
class TaskManager:
    ...
    def __init__(self, max_concurrent_tasks: int = 3, task_timeout: float = 300.0, cleanup_interval: float = 60.0):
    ...
    def submit_task(
        ...
        heapq.heappush(self.priority_queue, TaskPriority(priority, task_id, time.time()))
    ...
    def list_tasks(self, status: Optional[TaskStatusEnum] = None, limit: int = 50, offset: int = 0) -> List[TaskStatus]:
        """
    ...
    def update_task_progress(
        self, task_id: str, current_step: int, total_steps: int, action: Optional[ActionResult] = None
    ):
    ...
    async def _worker_loop(self):
        ...
        while self._running:
            try:
                ...
                while len(self.priority_queue) > 0 and len(self.active_tasks) < self.max_concurrent_tasks:
                    ...
                    atask.add_done_callback(lambda t, tid=task_id: self._task_done(tid))
