...
class BrowserController:
    ...
    async def launch(self) -> "BrowserController":
        ...
        try:
            ...
            browser_type = browser_types.get(self.browser_config.browser_type, self._playwright.chromium)
            ...
            # Launch browser
            self._browser = await browser_type.launch(headless=self.browser_config.headless, args=launch_args)
    ...
    async def get_frames(self) -> List[str]:
        ...
        return [
            await frame.get_attribute("id") or await frame.get_attribute("name") or f"iframe[{i}]"
