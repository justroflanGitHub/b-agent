...
class LLMClient:
    ...
    async def chat_with_image(
        self, text: str, image: Union[bytes, str], system_prompt: Optional[str] = None, **kwargs
    ) -> ChatResponse:
        """
        ...
        message = ChatMessage(role=MessageRole.USER, content=text, images=[image_b64])

        return await self.chat([message], system_prompt=system_prompt, **kwargs)

    async def chat_stream(
        self, messages: List[ChatMessage], system_prompt: Optional[str] = None, **kwargs
        ...
        try:
            async with self._session.post(
                self.chat_endpoint, json=payload, headers={"Content-Type": "application/json"}
            ) as response:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(
                        f"LLM request failed (attempt {attempt + 1}/{max_retries}): {e}. " f"Retrying in {delay}s..."
                    )
...
class VisionClient(LLMClient):
    ...
    async def analyze_screenshot(self, screenshot: bytes, prompt: str, **kwargs) -> ChatResponse:
        """
    ...
    async def detect_elements(
        self, screenshot: bytes, element_types: Optional[List[str]] = None, **kwargs
    ) -> Dict[str, Any]:
        """
...
"""
    ...
    async def predict_action(
        self, screenshot: bytes, task: str, available_actions: Optional[List[str]] = None, **kwargs
    ) -> Dict[str, Any]:
        """
...
"""
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                logger.info(
                    f"🎯 Coordinate tool: ({result.get('x')}, {result.get('y')}) confidence={result.get('confidence', 0)}"
                )
                return result
