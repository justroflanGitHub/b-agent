...
class UserPreferenceStore:
    ...
    def get_category(self, category: str) -> Dict[str, Any]:
        ...
        return {
            key: self._preferences[key].value
            for key in self._category_index[category]
            if key in self._preferences
        }
    ...
    def delete(self, key: str) -> bool:
        ...
        if pref.category in self._category_index:
            self._category_index[pref.category] = [
                k for k in self._category_index[pref.category] if k != key
            ]
...
class CorrectionFeedbackLearner:
    ...
    def _calculate_context_similarity(
        ...
        value_matches = sum(
            1 for k in common_keys if ctx1.get(k) == ctx2.get(k)
        )
    ...
    def _add_correction(self, feedback: CorrectionFeedback):
        ...
        if len(self._corrections) >= self.max_corrections:
            oldest_id = min(
                ...
                key=lambda x: self._corrections[x].timestamp
            )
    ...
    def _remove_correction(self, feedback_id: str):
        ...
        if action_type in self._action_index:
            self._action_index[action_type] = [
                fid for fid in self._action_index[action_type]
                if fid != feedback_id
            ]
    ...
    def record_outcome(self, feedback_id: str, success: bool):
        ...
        if success:
            feedback.success_rate = (
                (feedback.success_rate * (total - 1) + 1) / total
            )
        else:
            feedback.success_rate = (
                (feedback.success_rate * (total - 1)) / total
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_success_rate": sum(
                c.success_rate for c in self._corrections.values()
            ) / len(self._corrections),
            "total_applications": sum(
                c.applied_count for c in self._corrections.values()
            ),
            "by_action_type": {
                k: len(v) for k, v in self._action_index.items()
            },
        }
...
class TaskTemplateManager:
    ...
    def _evict_worst_template(self):
        ...
        if not candidates:
            ...
            oldest_id = min(
                ...
                key=lambda x: self._templates[x].created_at
            )
    ...
    def get_templates_by_tag(self, tag: str) -> List[TaskTemplate]:
        ...
        return [
            self._templates[tid]
            for tid in self._tag_index[tag]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_success_rate": sum(
                t.success_rate for t in self._templates.values()
            ) / len(self._templates),
...
class ConversationMemorySystem:
    ...
    def get_action_correction(
        ...
        correction = self.feedback_learner.get_correction_for_action(
            context, proposed_action
        )
        
        if correction:
            return self.feedback_learner.apply_correction(
                correction, proposed_action
            )
        
        return None
    
    def find_task_templates(
        self,
        goal: str
    ) -> List[Tuple[TaskTemplate, float]]:
