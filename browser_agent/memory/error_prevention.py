...
class BehaviorBaseline:
    ...
    def update(self, new_value: float):
        ...
        if self.sample_count > 1:
            ...
            new_variance = old_variance + ((new_value - old_mean) * (new_value - self.mean) - old_variance) / self.sample_count
            ...
            if baseline_key in self._baselines:
                ...
                if len(self._baselines) > self.max_baselines:
                    oldest_key = min(
                        ...
                        key=lambda k: self._baselines[k].last_updated
                    )
...
class HeuristicWarningSystem:
    ...
    def _check_navigation_risk(self, context: Dict[str, Any]) -> Optional[Warning]:
        ...
        risky_patterns = [
            ...
            "/confirm", "/verify", "/payment", "/checkout"
        ]
    ...
    def add_custom_heuristic(
        self,
        heuristic: Callable[[Dict[str, Any]], Optional[Warning]]
    ):
...
class SuspiciousStateHandler:
    ...
    def handle_suspicious_state(
        ...
        logger.warning(
            f"Suspicious state detected: {len(anomalies)} anomalies, "
    ...
    def _add_state(self, state: SuspiciousState):
        ...
        if len(self._suspicious_states) >= self.max_states:
            oldest_hash = min(
                self._suspicious_states.keys(),
                key=lambda h: self._suspicious_states[h].timestamp
            )
    ...
    def get_unreviewed_states(self) -> List[SuspiciousState]:
        ...
        return [
            ...
            if h in self._suspicious_states
            and not self._suspicious_states[h].reviewed
        ]
    ...
    def mark_reviewed(self, state_hash: str, notes: str = ""):
        ...
        if state_hash in self._suspicious_states:
            ...
            self._review_queue = [
                h for h in self._review_queue if h != state_hash
            ]
...
class PreActionRiskAssessment:
    ...
    def record_outcome(
        ...
        if len(self._action_outcomes[action_type]) > self.history_size:
            self._action_outcomes[action_type] = (
                self._action_outcomes[action_type][-self.history_size:]
            )
    
    def add_risk_factor(
        self,
        factor_fn: Callable[[Dict[str, Any]], Tuple[float, str]]
    ):
...
class ErrorPreventionSystem:
    ...
    def __init__(
        ...
        self.suspicious_handler = suspicious_handler or SuspiciousStateHandler(
    ...
    def record_action_result(
        ...
        self.risk_assessment.record_outcome(
