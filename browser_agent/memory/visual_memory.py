...
class EmbeddingVector:
    ...
    def to_bytes(self) -> bytes:
        ...
        return struct.pack(f"fi{self.dimension}f", self.timestamp, self.dimension, *self.vector)
...
class ScreenshotEmbeddingCache:
    ...
    def get_or_create_embedding(
        self, screenshot: bytes, compute_fn: Optional[Callable[[], List[float]]] = None
        ...
        new_embedding = EmbeddingVector(
            vector=new_embedding_vector, dimension=self.embedding_dimension, source_hash=screenshot_hash
        )
    ...
    def _add_to_cache(self, screenshot_hash: str, embedding: EmbeddingVector):
        ...
        while len(self._cache) >= self.max_size:
            ...
            # Clean up hash index
            self._hash_index = {k: v for k, v in self._hash_index.items() if v != oldest_key}
    ...
    def find_similar(
        self, embedding: EmbeddingVector, top_k: int = 5, threshold: float = 0.8
    ...
    def save_to_disk(self):
        ...
        data = {
            "embeddings": [(key, emb.to_bytes()) for key, emb in self._cache.items()],
...
class UIStateDetector:
    ...
    def __init__(
        self, embedding_cache: ScreenshotEmbeddingCache, state_similarity_threshold: float = 0.85, max_states: int = 500
    ):
        """
    ...
    def detect_state(
        self, screenshot: bytes, url: str, title: str = "", element_count: int = 0, interactive_count: int = 0
    ) -> Tuple[UIState, bool]:
        """
    ...
    def _find_similar_states(self, embedding: EmbeddingVector, url: Optional[str] = None) -> List[UIState]:
    ...
    def _add_state(self, state: UIState):
        ...
        if len(self._states) >= self.max_states:
            oldest_id = min(self._states.keys(), key=lambda x: self._states[x].last_visit)
    ...
    def _remove_state(self, state_id: str):
        ...
        if state.url in self._url_state_index:
            self._url_state_index[state.url] = [s for s in self._url_state_index[state.url] if s != state_id]
    ...
    def get_states_for_url(self, url: str) -> List[UIState]:
        ...
        if url not in self._url_state_index:
            return []
        return [self._states[sid] for sid in self._url_state_index[url] if sid in self._states]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        avg_actions = (
            sum(len(s.actions_taken) for s in self._states.values()) / len(self._states) if self._states else 0
        )
        ...
        return {
            ...
            "total_transitions": sum(len(targets) for targets in self._state_transitions.values()),
...
class NavigationPatternLearner:
    ...
    def __init__(
        self, max_patterns: int = 200, min_success_rate: float = 0.6, pattern_similarity_threshold: float = 0.8
    ):
        """
    ...
    def learn_pattern(
        ...
        existing_pattern = self._find_similar_pattern(source_state.state_id, target_state.state_id, action_sequence)
    ...
    def _find_similar_pattern(
        self, source_state_id: str, target_state_id: str, action_sequence: List[Dict[str, Any]]
    ) -> Optional[NavigationPattern]:
        ...
        for pattern in candidates:
            if self._action_sequences_similar(action_sequence, pattern.action_sequence):
                return pattern

        return None

    def _action_sequences_similar(self, seq1: List[Dict[str, Any]], seq2: List[Dict[str, Any]]) -> bool:
    ...
    def _evict_worst_pattern(self):
        ...
        worst_id = min(self._patterns.keys(), key=lambda x: self._patterns[x].success_rate)
        ...
        if pattern.source_state_id in self._source_index:
            self._source_index[pattern.source_state_id] = [
                p for p in self._source_index[pattern.source_state_id] if p != worst_id
            ]
        if pattern.target_state_id in self._target_index:
            self._target_index[pattern.target_state_id] = [
                p for p in self._target_index[pattern.target_state_id] if p != worst_id
            ]
    ...
    def get_patterns_for_source(self, source_state_id: str) -> List[NavigationPattern]:
        ...
        patterns = [self._patterns[pid] for pid in self._source_index[source_state_id] if pid in self._patterns]
    ...
    def get_patterns_for_target(self, target_state_id: str) -> List[NavigationPattern]:
        ...
        patterns = [self._patterns[pid] for pid in self._target_index[target_state_id] if pid in self._patterns]

        return sorted(patterns, key=lambda p: p.success_rate, reverse=True)

    def suggest_navigation(
        self, source_state: UIState, target_url: Optional[str] = None
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_success_rate": sum(p.success_rate for p in self._patterns.values()) / len(self._patterns),
...
class DynamicElementReidentifier:
    ...
    def __init__(self, max_elements: int = 1000, position_tolerance: int = 20):
        """
    ...
    def _positions_similar(self, pos1: Tuple[int, int, int, int], pos2: Tuple[int, int, int, int]) -> bool:
    ...
    def _add_element(self, element: DynamicElement):
        ...
        if len(self._elements) >= self.max_elements:
            oldest_id = min(self._elements.keys(), key=lambda x: self._elements[x].last_seen)
    ...
    def _remove_element(self, element_id: str):
        ...
        if element.content_hash in self._content_index:
            self._content_index[element.content_hash] = [
                eid for eid in self._content_index[element.content_hash] if eid != element_id
            ]
    ...
    def find_by_content(self, content: str) -> List[DynamicElement]:
        ...
        return [self._elements[eid] for eid in self._content_index[content_hash] if eid in self._elements]

    def find_by_position(self, position: Tuple[int, int, int, int]) -> List[DynamicElement]:
    ...
    def get_elements_for_state(self, state_id: str) -> List[DynamicElement]:
        ...
        return [e for e in self._elements.values() if state_id in e.state_associations]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_appearances": sum(e.appearance_count for e in self._elements.values()) / len(self._elements),
            "elements_with_variations": sum(1 for e in self._elements.values() if e.position_variations),
        }
...
class VisualMemorySystem:
    ...
    def __init__(
        ...
        self.state_detector = state_detector or UIStateDetector(embedding_cache=self.embedding_cache)
    ...
    def process_screenshot(
        ...
        if self._current_state and self._current_state.state_id != state.state_id:
            self.state_detector.record_transition(self._current_state.state_id, state.state_id)
    ...
    def record_action(self, action: Dict[str, Any]):
        ...
        if self._current_state:
            ...
            self.state_detector.record_action(self._current_state.state_id, action_str, "pending")
    ...
    def end_navigation(self, success: bool, target_state: Optional[UIState] = None):
        ...
        if self._current_state and target_state and self._current_action_sequence:
    ...
    def track_element(
        self, selector: str, content: str, position: Tuple[int, int, int, int], element_type: str
    ) -> DynamicElement:
        ...
        return self.element_reidentifier.track_element(
            selector=selector, content=content, position=position, element_type=element_type, state_id=state_id
        )
