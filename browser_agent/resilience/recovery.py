...
class RecoveryOrchestrator:
    ...
    def add_manual_intervention_callback(
        self,
    ...
    def remove_manual_intervention_callback(
        self,
        callback: Callable[[RecoveryResult], None]
    ) -> None:
    ...
    async def attempt_state_stack_rollback(
        ...
        if frame:
            ...
            try:
                ...
                # Restore scroll
                await page.evaluate(
                    f"window.scrollTo({frame.state.scroll_x}, {frame.state.scroll_y})"
                )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        success_rate = (
            self._success_count / self._recovery_count * 100
            if self._recovery_count > 0 else 0
        )
