...
from .checkpoint import BrowserState, Checkpoint, CheckpointType
...
class StateStack:
    ...
    def get_all_branches(self) -> Dict[str, int]:
        ...
        return {
            name: len(frames)
            for name, frames in self._branches.items()
        }
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        branch_point_count = sum(
            1 for f in self._frames.values() if f.is_branch_point
        )
