...
class BaseSkill(ABC, Generic[T]):
    ...
    async def _execute_with_retry(
        ...
        for attempt in range(retries + 1):
            try:
                self._logger.info(f"Executing skill {self.name} (attempt {attempt + 1}/{retries + 1})")
                ...
                if attempt < retries:
                    ...
                    self._logger.warning(f"Skill {self.name} failed, retrying in {wait_time}s: {result.error}")
