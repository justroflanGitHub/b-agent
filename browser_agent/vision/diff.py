...
class VisualDiff:
    ...
    def __init__(self, similarity_threshold: float = 0.95, change_threshold: float = 0.1, region_size: int = 50):
        """
    ...
    def compare(self, before: bytes, after: bytes) -> VisualDiffResult:
        """
        ...
        try:
            ...
            return VisualDiffResult(
                are_similar=False, similarity_score=0.0, changed_regions=[], total_pixels_changed=0, total_pixels=0
            )
    ...
    def _detect_regions(self, changed_mask: "np.ndarray", diff_values: "np.ndarray") -> List[DiffRegion]:
    ...
    def _generate_diff_image(self, before: "np.ndarray", after: "np.ndarray", diff: "np.ndarray") -> Optional[str]:
...
def create_visual_diff(similarity_threshold: float = 0.95, change_threshold: float = 0.1) -> VisualDiff:
    """Create a visual diff instance with specified thresholds."""
    return VisualDiff(similarity_threshold=similarity_threshold, change_threshold=change_threshold)
